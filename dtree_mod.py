...
def find_best_split(X, y, loss, min_samples_leaf, max_features):
    ...
    # Randomly chooses from features
    feature_list = np.random.choice(range(len(X.T)), max(
        1, int(max_features * len(X.T))), replace=False)

    for col_i in feature_list:
        candidates = np.unique(X[:, col_i])

        if len(candidates) <= 11:
            split_candidates = candidates
        else:
            split_candidates = candidates[np.random.randint(
                0, len(candidates), size=11)]
...
class ClassifierTree(DecisionTree):
    ...
    def create_leaf(self, y):
        """
        Return a new LeafNode for classification, passing y and returning y for prediction
        """
        return LeafNode(y, y)
